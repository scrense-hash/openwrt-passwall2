...
function gen_config(var)
	...
	if dns_listen_port then
		...
		if remote_dns_doh_url then
			if version_ge_1_12_0 then
				local doh_path = remote_dns_doh_url:match("https?://[^/]+(/.+)") or "/dns-query"
				remote_server.type = "https"
				remote_server.server = remote_dns_doh_ip or remote_dns_doh_host
		...
		if remote_server.address or remote_server.server then
			...
			for index, value in ipairs(dns_domain_rules) do
				if value.outboundTag and (value.domain or value.domain_suffix or value.domain_keyword or value.domain_regex or value.rule_set) then
					...
					if value.outboundTag ~= "block" and value.outboundTag ~= "direct" then
						...
						if value.outboundTag ~= COMMON.default_outbound_tag and (remote_server.address or remote_server.server) and remote_dns_detour ~= "direct" then
